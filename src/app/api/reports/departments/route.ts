...
import { getDepartmentsReport } from '@/lib/reports';
...
export async function GET(req: NextRequest) {
  ...
  try {
    const report = await getDepartmentsReport(targetCompanyId);
    return NextResponse.json(report);
  } catch (error) {
    console.error('Error fetching departments report:', error);
    return NextResponse.json(
      { error: 'Error interno al obtener reporte de departamentos' },
      { status: 500 }
    );
  }
}
