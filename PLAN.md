...
**Estado (construido y probado de extremo a extremo contra Postgres real el 2026-09-11)**: crear/editar/publicar/archivar cursos, CRUD de categorías, módulos y lecciones (crear/editar/eliminar), búsqueda y filtros del catálogo, y subir VIDEO/PPTX/PDF/IMAGE o agregar un LINK con reproducción inline — todo respetando el aislamiento multi-empresa (probado explícitamente: un colaborador de otra empresa recibe 404 en la página del curso y 403 al intentar el streaming directo del archivo). Todas las rutas de mutación (`POST`/`PATCH`/`DELETE`) fuera del matcher de `middleware.ts` validan `isTrustedOrigin()` individualmente (SPEC.md 1.9).

- [x] CRUD de categorías de cursos — `src/app/(dashboard)/courses/categories/`, `src/app/api/categories/`; crear/editar: ADMIN y MANAGER, eliminar: solo ADMIN
- [x] CRUD de cursos (crear, editar, publicar, archivar) con asignación a empresa(s) — `src/app/api/courses/route.ts` y `src/app/api/courses/[courseId]/route.ts`. Archivar es soft-delete (`status: ARCHIVED`, SPEC.md 2.6), nunca borra el curso
- [x] Módulos y lecciones (estructura jerárquica), con editar/eliminar — `src/app/api/modules/[moduleId]/route.ts`, `src/app/api/lessons/[lessonId]/route.ts`
...
- [x] Catálogo de cursos (búsqueda + filtros), respetando aislamiento por empresa — `src/components/courses/CourseFilters.tsx`
- [x] Limpieza de archivos huérfanos: al eliminar un módulo o una lección, `deleteResourceFiles()` (`src/lib/uploads.ts`) borra también los archivos físicos de sus recursos, no solo las filas en la base de datos — probado explícitamente (se subió un video, se eliminó la lección, se confirmó que el archivo desapareció de `UPLOADS_DIR`)
- **Pendiente, no bloqueante**: no existe todavía un endpoint para eliminar un recurso individual sin borrar toda la lección; reordenar módulos/lecciones por drag-and-drop (el campo `position` existe pero no hay UI para cambiarlo).
